	...
	function loadFile(remote, url) {
		return remote
			...
			.executeAsync(function (done) {
				require(["delite/register", "dojo/domReady!"].concat(testConfig.deps || []), function (register) {
					register.parse(); //TODO: cannot do parse().then(..) as the returned promise is never resolved in this case
					testConfig.callback.apply(null, Array.prototype.slice.call(arguments, 2));
					done();
				});
			});
	}
	...
	registerSuite({
		...
		"Checkbox behavior": function () {
			...
			return loadFile(remote, "./CheckBox.html")
				...
				.getAttribute("name")
				.then(function (v) {
					assert.equal(v, "cb1", "Unexpected focused element after 1st TAB.");
				})
				.end()
