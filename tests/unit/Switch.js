define([
	...
	var commonSuite = {
		...
		"Default State": function () {
			...
			assert.equal(sw1.name, elt.getAttribute("name"), "Unexpected 'name' attribute value for wrapped input.");
			assert.equal(sw1.value, elt.getAttribute("value"), "Unexpected 'value' attribute value for wrapped input.");
		...
		"Parameter init": function () {
			...
			assert.equal(sw2.name, elt.getAttribute("name"), "Unexpected 'name' attribute value for wrapped input.");
			assert.equal(sw2.value, elt.getAttribute("value"), "Unexpected 'value' attribute value for wrapped input.");
		...
		"checked": function () {
			var sw1 = document.getElementById("sw1"),
				...
				old = sw1.checked;
			sw1.checked = !old;
			assert.equal(sw1.checked, !old, "Unexpected value for 'checked' property after set.");
			assert.equal(sw1.checked, inp.checked, "Unexpected value for 'checked' property of wrapped input.");
		},

		value: function () {
			var sw1 = document.getElementById("sw1");
			sw1.value = "foo";
			assert.equal(sw1.firstChild.getAttribute("value"), "foo", "Unexpected value for 'value' attribute.");
		},

		destroy: function () {
			var sw1 = document.getElementById("sw1"),
				handlers = {},
				a1 = advise.after(sw1._knobGlassNode, "addEventListener", function (args) {
					handlers[args[0]] = args[1];
				}),
				a2 = advise.after(sw1._knobGlassNode, "removeEventListener", function (args) {
					delete handlers[args[0]];
				});
			// simulate pointerdown to trigger listeners registration
			var evt = document.createEvent("MouseEvents");
			evt.initMouseEvent("mousedown", true, true, window,
				0, 0, 0, 0, 0, false, false, false, false, 0, null);
			sw1._knobGlassNode.dispatchEvent(evt);
			evt.initMouseEvent("mouseup", true, true, window,
				0, 0, 0, 0, 0, false, false, false, false, 0, null);
			sw1._knobGlassNode.dispatchEvent(evt);
			sw1.destroy();
			assert.equal(Object.keys(handlers).length, 0, "Unexpected listeners after destroy.");
			assert.isNull(sw1._pHandlers, "Event handlers internal array not freed.");
			a1.unadvise();
			a2.unadvise();
		},
